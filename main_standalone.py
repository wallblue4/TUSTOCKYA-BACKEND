...
def search_products_in_real_inventory(model_name: str, limit: int = 5):
    """
    Función original con logs detallados
    """
    print(f"🔍 [DEBUG] search_products_in_real_inventory called with model_name: '{model_name}', limit: {limit}")
    
    try:
        if USE_POSTGRESQL:
            print(f"🔍 [DEBUG] Using PostgreSQL connection")
            import psycopg2
            import psycopg2.extras
            conn = psycopg2.connect(DB_PATH)
            cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            params = (f'%{model_name}%', f'%{model_name}%', f'%{model_name}%', limit)
            print(f"🔍 [DEBUG] PostgreSQL Query: {query}")
            ...
            print(f"🔍 [DEBUG] Using SQLite connection")
            conn = sqlite3.connect(DB_PATH)
            conn.row_factory = sqlite3.Row
        ...
        if products:
            for i, product in enumerate(products):
                ref_code = product.get('reference_code', 'NO_CODE')
                description = product.get('description', 'NO_DESC')
                total_available = product.get('total_available', 0)
                print(f"   📦 [DEBUG] Product {i+1}: {ref_code} - {description} - Stock: {total_available}")
                    ...
                    for pair in size_pairs:
                        if '/' in pair:
                            ...
                            stock_by_size.append({
                                ...
                                "location": product.get('location_name', 'Unknown')
                            })
                    product['parsed_stock'] = stock_by_size
                else:
                    product['parsed_stock'] = []
        else:
            print(f"   ❌ [DEBUG] No products found matching '{model_name}'")
        
        return products
        
    except Exception as e:
        print(f"❌ [ERROR] search_products_in_real_inventory: {e}")
        import traceback
        traceback.print_exc()
        return []
...
def merge_classification_with_inventory(classification_result, user_location_id):
    ...
    try:
        for rank, result in enumerate(classification_result['results'][:3], 1):
            ...
            print(f"🔍 [DEBUG] Processing result {rank}: model='{model_name}', original_brand='{original_brand}'")
            
            # ✅ LLAMAR EXPLÍCITAMENTE A LA BÚSQUEDA ORIGINAL
            print(f"🔄 [DEBUG] Calling search_products_in_real_inventory with: '{model_name}'")
            real_products = search_products_in_real_inventory(model_name)
            print(f"📊 [DEBUG] search_products_in_real_inventory returned: {len(real_products) if real_products else 0} products")
            
            # Log de los productos encontrados
            if real_products:
                for i, product in enumerate(real_products):
                    print(f"   📦 [DEBUG] Product {i+1}: {product.get('reference_code', 'NO_CODE')} - {product.get('description', 'NO_DESC')}")
            else:
                print(f"   ❌ [DEBUG] No products found in search_products_in_real_inventory")
            ...
            if real_products and len(real_products) > 0:
                print(f"✅ [DEBUG] FOUND PRODUCTS! Processing first product...")
                
                # ✅ USAR EL PRIMER PRODUCTO ENCONTRADO
                real_product = real_products[0]
                reference_code = real_product.get('reference_code')
                
                print(f"📍 [DEBUG] Selected product: {reference_code}")
                print(f"📍 [DEBUG] Product details: {real_product}")
                ...
                if reference_code:
                    ...
                    if all_locations and len(all_locations) > 0:
                        print(f"✅ [DEBUG] FOUND LOCATIONS! Processing {len(all_locations)} locations...")
                        
                        # Log de ubicaciones encontradas
                        for i, loc in enumerate(all_locations):
                            loc_id = loc.get('location_info', {}).get('location_id', 'NO_ID')
                            loc_name = loc.get('location_info', {}).get('location_name', 'NO_NAME')
                            stock = loc.get('stock_info', {}).get('total_stock', 0)
                            print(f"   📍 [DEBUG] Location {i+1}: ID={loc_id}, Name='{loc_name}', Stock={stock}")
                        
                        # ✅ SEPARAR POR UBICACIÓN DEL USUARIO
                        current_location = []
                        other_locations = []
                        
                        print(f"🔍 [DEBUG] User location_id: {user_location_id}")
                        
                        for location_data in all_locations:
                            location_id = location_data.get('location_info', {}).get('location_id')
                            location_name = location_data.get('location_info', {}).get('location_name', 'Unknown')
                            
                            print(f"   🔍 [DEBUG] Checking location: ID={location_id}, Name='{location_name}'")
                            
                            if location_id == user_location_id:
                                current_location.append(location_data)
                                print(f"   ✅ [DEBUG] Added to current_location: {location_name}")
                            else:
                                other_locations.append(location_data)
                                print(f"   📦 [DEBUG] Added to other_locations: {location_name}")
                        
                        # ✅ CALCULAR DISPONIBILIDAD
                        current_stock = sum(loc.get('stock_info', {}).get('total_stock', 0) for loc in current_location)
                        other_stock = sum(loc.get('stock_info', {}).get('total_stock', 0) for loc in other_locations)
                        total_stock = current_stock + other_stock
                        
                        print(f"📊 [DEBUG] Stock summary:")
                        print(f"   📍 Current location stock: {current_stock}")
                        print(f"   📦 Other locations stock: {other_stock}")
                        print(f"   📊 Total stock: {total_stock}")
                        
                        # Tallas disponibles
                        current_sizes = []
                        for loc in current_location:
                            for size_info in loc.get('stock_info', {}).get('available_sizes', []):
                                if size_info.get('quantity_stock', 0) > 0:
                                    current_sizes.append(size_info.get('size'))
                        
                        other_sizes = []
                        for loc in other_locations:
                            for size_info in loc.get('stock_info', {}).get('available_sizes', []):
                                if size_info.get('quantity_stock', 0) > 0 and size_info.get('size') not in current_sizes:
                                    other_sizes.append(size_info.get('size'))
                        ...
                        print(f"🎯 [DEBUG] Final recommendation: {recommended_action}")
                        
                        # ✅ CREAR RESULTADO CON DATOS REALES
                        merged_result = {
                            "rank": rank,
                            ...
                            "reference": {
                                "code": reference_code,  # ✅ CÓDIGO REAL
                                "brand": display_brand,
                                "model": model_name,
                                "color": real_product.get('color_info') or "Varios",
                                "description": real_product.get('description', f"{display_brand} {model_name}"),
                                "photo": real_product.get('image_url') or f"https://via.placeholder.com/300x300?text={display_brand}+{model_name}"
                            },
                            "availability": {
                                "summary": {
                                    "current_location": {
                            ...
                            "pricing": {
                                "unit_price": float(real_product.get('unit_price', 0)),
                                "box_price": float(real_product.get('box_price', 0)),
                                "has_pricing": True
                            },
                        ...
                        print(f"✅ [DEBUG] SUCCESS! Added result {rank} with REAL reference code: {reference_code}")
                        continue  # Ir al siguiente resultado
                        
                    else:
                        print(f"❌ [DEBUG] No locations found for reference_code: {reference_code}")
                else:
                    print(f"❌ [DEBUG] No reference_code in real_product")
            else:
                print(f"❌ [DEBUG] No real products found, falling back to basic classification")
            
            # ✅ FALLBACK: SIN INVENTARIO
            print(f"📝 [DEBUG] Creating basic classification result for rank {rank}")
...
async def get_pending_receptions(current_user = Depends(get_current_user)):
    """Ver productos entregados pendientes de confirmación de recepción - MEJORADO con imagen y precio"""
    ...
    # Calcular tiempo desde entrega y agregar información completa del producto
    for item in pending:
        if item['delivered_at']:
            ...
            item['hours_since_delivery'] = 0
            item['requires_urgent_confirmation'] = False
        ...
        item['product_info'] = {
            ...
            "size": item['size'],
            "quantity": item['quantity'],
            "color": item['product_color'] or "Varios",
            "description": item['product_description'] or f"{item['brand']} {item['model']}",
            "unit_price": float(item['product_unit_price']) if item['product_unit_price'] else 0.0,
            "box_price": float(item['product_box_price']) if item['product_box_price'] else 0.0,
            "total_value": (float(item['product_unit_price']) if item['product_unit_price'] else 0.0) * item['quantity'],
            "image_url": item['product_image'] or f"https://via.placeholder.com/300x200?text={item['brand'].replace(' ', '+')}+{item['model'].replace(' ', '+')}"
        }
        
        # Información de entrega
        item['delivery_info'] = {
            "delivered_by": "Tú mismo" if item['pickup_type'] == 'seller' else f"{item['courier_first_name']} {item['courier_last_name']}" if item['courier_first_name'] else "Corredor",
            "delivered_from": item['source_location_name'],
            "delivery_method": "Self-pickup" if item['pickup_type'] == 'seller' else "Courier delivery",
            "delivered_at": item['delivered_at'],
            "hours_ago": round(item['hours_since_delivery'], 1)
        }
        
        # Estado de urgencia
        if item['requires_urgent_confirmation']:
            item['urgency_status'] = {
                "level": "high",
                "message": f"Pendiente por {round(item['hours_since_delivery'], 1)} horas - Confirma pronto",
                "color": "red"
            }
        elif item['hours_since_delivery'] > 1:
            item['urgency_status'] = {
                "level": "medium", 
                "message": f"Pendiente por {round(item['hours_since_delivery'], 1)} horas",
                "color": "orange"
            }
        else:
            item['urgency_status'] = {
                "level": "normal",
                "message": "Recién entregado",
                "color": "green"
            }
    ...
    return {
        ...
        "total_value": sum(p['product_info']['total_value'] for p in pending),
        "summary": {
            "self_pickup": len([p for p in pending if p['pickup_type'] == 'seller']),
            "courier_delivery": len([p for p in pending if p['pickup_type'] == 'corredor']),
            "average_hours_pending": round(sum(p['hours_since_delivery'] for p in pending) / len(pending), 1) if pending else 0
        }
    }
        ...
        if status == 'pending':
            ...
            if is_self_pickup:
                transfer['status_info'] = {
                    ...
                    "description": f"Aceptada por {warehouse_keeper} - Ve a recoger",
                    "detail": "El bodeguero te entregará el producto cuando llegues",
                    "action_required": None,
                    "next_step": "Ve a la bodega cuando puedas",
            ...
            transfer['status_info'] = {
                ...
                "description": f"{'Recibiste' if is_self_pickup else 'Entregado'} - confirma recepción",
...
async def get_accepted_transfer_requests(current_user = Depends(get_current_user)):
    ...
    for request in requests:
        ...
        if request['pickup_type'] == 'seller' and request['status'] == 'accepted':
            ...
            request['action_type'] = 'deliver_to_vendor'
            request['action_endpoint'] = f"/api/v1/warehouse/deliver-to-vendor/{request['id']}"
        elif request['pickup_type'] == 'corredor' and request['status'] == 'accepted':
    ...
    return {
        ...
        "count": len(requests),
        "breakdown": {
            "waiting_courier": len([r for r in requests if r['status'] == 'accepted' and r['pickup_type'] == 'corredor']),
            "ready_for_vendor": len([r for r in requests if r['status'] == 'accepted' and r['pickup_type'] == 'seller']),
...
@app.post("/api/v1/warehouse/deliver-to-vendor/{transfer_id}")
async def deliver_to_vendor(
    ...
    try:
        if delivered:
            ...
            if USE_POSTGRESQL:
                cursor.execute('''
                    ...
                    WHERE product_id = (
                        SELECT p.id FROM products p 
                        WHERE p.reference_code = %s 
                        AND UPPER(p.location_name) = UPPER((SELECT name FROM locations WHERE id = %s))
                    ) 
                    AND size = %s
                ...
                conn.execute('''
                    ...
                    WHERE product_id = (
                        SELECT p.id FROM products p 
                        WHERE p.reference_code = ? 
                        AND UPPER(p.location_name) = UPPER((SELECT name FROM locations WHERE id = ?))
                    ) 
                    AND size = ?
                ''', (request['quantity'], request['sneaker_reference_code'], 
                      request['source_location_id'], request['size']))
            ...
            conn.commit()
            message = "Producto entregado al vendedor exitosamente"
            ...
            if USE_POSTGRESQL:
                cursor.execute('''
                    ...
                    WHERE id = %s
                ''', (f"Problema en entrega: {notes}", transfer_id))
            else:
                conn.execute('''
                    UPDATE transfer_requests 
            ...
            conn.commit()
            message = "Entrega marcada con problemas"
        ...
        return {
            ...
            "delivered_at": timestamp,
            "inventory_updated": delivered,
            "next_step": "Vendedor debe confirmar recepción" if delivered else "Revisar problemas reportados",
            "delivered_by": f"{current_user['first_name']} {current_user['last_name']}"
        }
...
async def accept_transfer_request_with_auto_assignment(
    ...
    """BG002: Aceptar solicitud con flujo simplificado para vendedores"""
    ...
    if acceptance.accepted:
        ...
        # CAMBIO: Mismo estado para ambos tipos
        new_status = "accepted"
        
        # Solo para tracking interno
        if request['pickup_type'] == 'seller':
            assigned_courier_id = request['requester_id']
            success_message = "Solicitud aceptada - Vendedor puede venir a recoger"
        else:
            assigned_courier_id = None
            success_message = "Solicitud aceptada - Esperando corredor"
    ...
    return {
        ...
        "next_step": "Preparar producto para entrega" if request['pickup_type'] == 'seller' and acceptance.accepted else "Esperar corredor" if acceptance.accepted else "Solicitud cancelada",
